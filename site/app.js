...
const moduleItems = [
  ["Dashboard 总览", "汇总待复核、DG 风险、异常、政策关注和港口风险。", "Live"],
  ...
  ["Global Trend Watch", "过去一周经济、政治、金融、贸易和供应链公开新闻观察。", "Live"],
  ["Vessel & Schedule Tracking", "船名模糊搜索、目的港、POL/POD、ETD/ETA。", "Live"],
  ...
  ["Port & Logistics Risk", "输入港口查看公开新闻风险、官方入口和手工判断建议。", "Live"],
...
async function loadTrends() {
  $("trendStatus").textContent = "正在加载过去一周公开新闻...";
  try {
    const data = await fetchJsonOrFallback("/.netlify/functions/global-trends", fallbackTrends());
    renderTrends(data);
  } catch (error) {
    renderTrends(fallbackTrends(error.message));
  }
}

function fallbackTrends(message = "") {
  return {
    ok: false,
    fallback: true,
        ...
        .map(
          (item) => `
            <article class="trend-card">
              ...
              <p>${escapeHtml(item.takeaway || "建议作为宏观背景观察。")}</p>
