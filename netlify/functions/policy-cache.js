...
const cacheKey = "ca:policy-cache";
...
exports.handler = async (event) => {
  ...
  if (!storageConfigured()) return json(200, { ok: false, storageConfigured: false, message: "Policy cache storage is not configured." });

  const raw = upstashConfigured() ? await redis(["GET", cacheKey]) : await localRead(cacheKey);
  if (!raw) return json(200, { ok: false, storageConfigured: true, message: "No policy cache yet." });
  try {
    return json(200, JSON.parse(raw));
  } catch {
    return json(200, { ok: false, storageConfigured: true, message: "Policy cache is invalid." });
  }
};
